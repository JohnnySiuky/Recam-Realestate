...
public class CreateListingCaseRequestValidator : AbstractValidator<CreateListingCaseRequest>
{
    public CreateListingCaseRequestValidator()
    {
        ...
        RuleFor(x => x.Latitude).InclusiveBetween(-90, 90).When(x => x.Price.HasValue);
        RuleFor(x => x.Longitude).InclusiveBetween(-180, 180).When(x => x.Price.HasValue);

        RuleFor(x => x.PropertyType).IsInEnum();
        RuleFor(x => x.SaleCategory).IsInEnum();
    }
}